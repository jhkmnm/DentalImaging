...
namespace DentalImaging.NewForm
{
    public partial class FormImageEdit : XtraForm
    {
        ...
        public FormImageEdit(ImgInfo imgInfo, CaseHistory history)
        {
            ...
            this.Text = $"{GetText("图片编辑")}-{imgInfo.FileName}";
        }
        ...
        private void InitDDL()
        {
            ...
            lineSource.AddRange(new[] {
                new TextValue { Text = GetText("直线"), Value = $"{(int)DrawType.Line}" },
                new TextValue { Text = GetText("箭头"), Value = $"{(int)DrawType.LineCap}"},
                new TextValue { Text = GetText("曲线"), Value = $"{(int)DrawType.Curve}" } });
            ...
            ellipseSource.AddRange(new[] {
                new TextValue { Text = GetText("空心圆"), Value = $"{(int)DrawType.Ellipse}" },
                new TextValue { Text = GetText("实心圆"), Value = $"{(int)DrawType.FillEllips}"}});
            ...
            rectangleSource.AddRange(new[] {
                new TextValue { Text = GetText("空心矩形"), Value = $"{(int)DrawType.Rectangle}" },
                new TextValue { Text = GetText("实心矩形"), Value = $"{(int)DrawType.FillRectangle}"} });
            ...
            textSource.AddRange(new[] {
                new TextValue { Text = GetText("左对齐"), Value = $"{(int)DrawType.LeftText}" },
                new TextValue { Text = GetText("右对齐"), Value = $"{(int)DrawType.RightText}"}});
        ...
        {
            CurrPen.Width = Convert.ToInt32(((TextValue)ddlPenSize.SelectedValue).Value);
        }
        ...
        private void SaveImg(bool isCopy = true)
        {
            ...
            {                
                ...
                var id = names.Max() + 1;
                var fileName = $"{id}";
                var imgPath = $"{dirName}\\{fileName}.bmp";
                ...
                File.WriteAllText($"{dirName}\\{fileName}.ini", copyInfo.ToJson());
        ...
        {
            var show = GetText("显示备注");
            var hidd = GetText("隐藏备注");
            var isShow = btnShowRemark.Text == show;
            btnShowRemark.Text = btnShowRemark.Text == show ? hidd : show;
